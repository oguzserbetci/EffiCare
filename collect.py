...
from dataloader.utils import BinnedEvent, Event, get_vocab


def parse_arguments():
    parser = argparse.ArgumentParser(description='collect predictions and insights for a trained model')
    parser.add_argument('wandb_id', default=None, help='experiment id to get the latest best model and config')
...
def run(wandb_id, data_path, dev, **params):
    ...
    val_loader = torch.utils.data.DataLoader(val_set, batch_size=1,
                                             collate_fn=partial(utils.pad_batch,
                                                                ...
                                                                labels=labels,
                                                                limit=None),
                                             num_workers=params['num_workers'] if not dev else 0, pin_memory=True)
...
config = utils.load_config(args.wandb_id)
